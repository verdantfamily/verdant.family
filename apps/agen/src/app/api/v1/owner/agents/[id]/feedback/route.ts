import { recordOwnerFeedback } from "../../../../../../lib/agents/autonomy";
...
export async function GET(
  ...
  try {
    ...
    return ok({ feedback: ctx.store.listFeedback(id) });
  } catch (error) {
    return fail(error);
  }
}
...
export async function POST(
  ...
  try {
    ...
    const feedback = recordOwnerFeedback(ctx.address, id, body, ctx.store);
    return ok({ feedback });
  } catch (error) {
    return fail(error);
  }
