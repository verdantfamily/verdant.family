 ...
 * token page draws. The root layout's `/og.jpg` is the brand card; this one is the
 * market's. Next attaches it as `og:image` and `twitter:image` for `/markets/[id]`.
 *
 ...
 *
 * The page does the same thing: Aeonik is licensed and dropped into `public/fonts`,
 * Inter Tight is the open stand-in. A share card that used a third face would look
 * like a different product sitting on top of a link to this one.
 */
async function loadFonts(): Promise<Face[]> {
  ...
  for (const [weight, file] of [
    ...
    try {
      ...
      // A card with the default face is still a card. A card that 500s because a
      // font CDN blinked is a broken link preview.
    }
  }
